...
library TransferHelper {
    ...
    address constant USDTAddr = 0xa614f803B6FD780986A42c78Ec9c7f77e6DeD13C; // todo !!!!!
...
contract JustSwapFactory{
    ...
    function initializeFactory(address template) public {
        require(exchangeTemplate == address(0), "exchangeTemplate already set"); require(template != address(0), "illegal template");
        exchangeTemplate = template;
    }


    function createExchange(address token) public returns (address) {
        require(token != address(0), "illegal token"); require(exchangeTemplate != address(0), "exchangeTemplate not set"); require(token_to_exchange[token] == address(0), "exchange already created");
    ...
    function getExchange(address token_address) public view returns(address exchange) {
        return token_to_exchange[token_address];
    }


    function getToken(address exchange_address) public view returns(address token) {
        return exchange_to_token[exchange_address];
    }


    function getTokenWithId(uint256 _id) public view returns(address) {
        return id_to_token[_id];
    }
}
